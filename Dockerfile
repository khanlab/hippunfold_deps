...
RUN wget https://github.com/Kitware/CMake/releases/download/v3.24.0-rc2/cmake-3.24.0-rc2-linux-aarch64.sh \
...
RUN apt-get --allow-releaseinfo-change  update && mkdir -p /usr/share/man/man1  &&  apt-get install -y curl tree unzip bc default-jre libgomp1 libpng-dev zlib1g-dev build-essential wget bzip2 ca-certificates gnupg2 squashfs-tools git graphviz-dev
...
RUN git clone https://github.com/stnava/ANTs.git && cd ANTs/ && \
mkdir antsbin && cd antsbin/ && cmake ../ && make -j4
